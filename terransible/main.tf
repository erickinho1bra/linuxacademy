...
resource "aws_security_group" "wp_dev_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["172.250.191.248/32"]
  }
  egress {
    from_port   = 0
    to_port     = 0
...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile superhero && ansible-playbook -i aws_hosts wordpress.yml && ansible-playbook -i aws_hosts configDev.yml"
  }
}
